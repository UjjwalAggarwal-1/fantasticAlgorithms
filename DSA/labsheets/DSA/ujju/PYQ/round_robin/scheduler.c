...
void visualize_round_robin(char *path) {
    ...
    for (int timer=0;;timer++){
        // printf("timer : %d\n", timer);
        //send new proc into queue first
        if ( pointer<num_processes && processes[pointer]->arrival == timer){
            add_last_linked_deque(ld, *(processes[pointer]));
            // printf("added %s\n",processes[pointer]->name);
            pointer++;
        }
        if(ld->list == NULL){
            break;
        }
        if(ld->list->head == NULL){
            break;
        }
        process * fproc = ld->list->head->process;
        fproc->remaining_time--;
        if (!(fproc->remaining_time)){
            fproc->turnaround = timer-(fproc->arrival);
            fproc->wait = fproc->turnaround - fproc->cpu_burst;
            // printf("removing \n");
            print_stats(*fproc);
            remove_first_linked_deque(ld, fproc);
            tq_pointer = 2;
        }else{
            if (tq_pointer==2){
                remove_first_linked_deque(ld, fproc);
                add_last_linked_deque(ld, *fproc);
                tq_pointer = 2;
            }
        }

        tq_pointer++;
        tq_pointer%=TIME_QUANTUM;
    }
